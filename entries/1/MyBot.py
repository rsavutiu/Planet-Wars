    ...
    if winRatio > 1.5:
        ...
        for p in enemyPlanets:
            ...
            for mp in pw.MyPlanets():
                defenseReq = max(DefenseRequiredForIncoming(pw, mp),
                                 int(ceil(.05 * GeneralDefenseRequired(pw, mp, enemyPlanets))))
                toSend = mp.NumShips()
        ...
        for p in enemyPlanets + neutralPlanets:
            ...
            if BreakEvenTurns(pw, p, dist) < 50:
                defenseReq = max(DefenseRequiredForIncoming(pw, p),
                                 int(ceil(.25 * GeneralDefenseRequired(pw, p, enemyPlanets))))
